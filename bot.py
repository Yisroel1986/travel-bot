        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if isinstance(data, dict):
                    # Предполагаем структуру data["data"] = [...]
...
#
# --- HELPER FUNCTION FOR SINGLE PRODUCT ---
#
def find_single_product(all_products, user_text: str):
    """
    Ищем, упомянул ли пользователь конкретный тур (по ID или по названию).
    Если нашли ровно один тур, возвращаем его.
    Если нет — возвращаем None.
    """
    user_text_lower = user_text.lower()

    # 1) Пытаемся найти ID вида "id 3" или просто число
    match_id = re.search(r'\b(\d+)\b', user_text_lower)
    if match_id:
        wanted_id = int(match_id.group(1))
        for p in all_products:
            if p.get("id") == wanted_id:
                return p

    # 2) Пытаемся найти по части названия
    #   (например, пользователь ввёл "Зоопарк Ніредьгаза" — ищем похожее)
    found_products = []
    for p in all_products:
        name_lower = p.get("name", "").lower()
        if name_lower and name_lower in user_text_lower:
            found_products.append(p)

    if len(found_products) == 1:
        return found_products[0]

    # Если совпадений нет или несколько — вернём None
    return None
...
async def typing_simulation(update: Update, text: str):
    """
    Отправляет эффект 'набираю сообщение' с увеличенной паузой,
    чтобы бот выглядел более “человечно”.
    """
    # Базовая задержка + динамическая часть в зависимости от длины текста
    base_delay = 4.0
    dynamic_delay = min(8.0, max(2.0, len(text)/60.0))
    total_delay = base_delay + dynamic_delay
...
async def details_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Шаг 1: Получаем все продукты (туры) из CRM
    all_products = fetch_all_products()

    # Шаг 2: Пробуем найти, упоминал ли пользователь конкретный тур (ID или название)
    user_text = update.message.text.lower()  # что пользователь ввёл
    single_product = find_single_product(all_products, user_text)

    # Если нашли ровно один конкретный тур:
    if single_product:
        pid = single_product.get("id", "?")
        pname = single_product.get("name", "No name")
        pprice = single_product.get("price", 0)
        pdesc = single_product.get("description", "")
        # Формируем короткое сообщение только об этом туре
        tours_info = (
            f"Ось деталі саме цього туру:\n"
            f"ID: {pid}\n"
            f"Назва: {pname}\n"
            f"Ціна: {pprice}\n"
            f"Опис: {pdesc if pdesc else 'Без опису'}\n"
        )
    else:
        # Если конкретный тур не найден, либо пользователь не уточнял,
    ...
    # Статичная часть описания (базовый текст)
    if choice == "cost":
        text = (
